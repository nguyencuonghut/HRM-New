...
class TemplateUploadService
{
    ...
    /**
     * Upload và validate DOCX template file cho appendix templates.
     *
     * @param UploadedFile $file
     * @param string $type appendix type (SALARY, ALLOWANCE, etc.)
     * @return array ['body_path' => 'templates/appendixes/{type}.docx']
     * @throws \RuntimeException
     */
    public static function uploadAppendixDocxTemplate(UploadedFile $file, string $type): array
    {
        // 1) Validate extension
        if ($file->getClientOriginalExtension() !== 'docx') {
            throw new \RuntimeException('File phải là định dạng .docx');
        ...
        // 4) Store file with type-based name
        $filename = strtolower($type) . '.docx';
        $dir = 'templates/appendixes';
    ...
    /**
     * Auto-detect placeholders và tạo default mappings cho appendix template
     *
     * @param ContractAppendixTemplate $template
     * @param string $docxPath Full path to DOCX file
     * @return array ['detected' => int, 'mapped' => int, 'unmapped' => int]
     */
    public static function createAppendixPlaceholderMappings(ContractAppendixTemplate $template, string $docxPath): array
    {
        $placeholders = PlaceholderExtractorService::extractFromDocx($docxPath);
        ...
        foreach ($placeholders as $placeholder) {
            ...
            // Check xem đã có mapping chưa
            $exists = ContractAppendixTemplatePlaceholderMapping::where('appendix_template_id', $template->id)
                ->where('placeholder_key', $placeholder)
                ->exists();
            ...
            if (isset($presets[$placeholder])) {
                ...
                // Unmapped - tạo mapping MANUAL để user config sau
                $dataSource = 'MANUAL';
                $sourcePath = null;
                $transformer = null;
                $defaultValue = '';
            ...
            ContractAppendixTemplatePlaceholderMapping::create([
                'id' => Str::uuid(),
                'appendix_template_id' => $template->id,
                'placeholder_key' => $placeholder,
                'data_source' => $dataSource,
                'source_path' => $sourcePath,
                'default_value' => $defaultValue,
