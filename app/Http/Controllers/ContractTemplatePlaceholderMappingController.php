...
class ContractTemplatePlaceholderMappingController extends Controller
{
     ...
     */
    public function index(ContractTemplate $template)
    {
     ...
     * Cập nhật 1 mapping
     */
    public function update(Request $request, ContractTemplate $template, ContractTemplatePlaceholderMapping $mapping)
    {
     ...
     */
    public function bulkUpdate(Request $request, ContractTemplate $template)
    {
        $this->authorize('update', $template);

        $validated = $request->validate([
            'mappings' => 'required|array',
            'mappings.*.id' => 'required|uuid|exists:contract_template_placeholder_mappings,id',
        ...
        foreach ($validated['mappings'] as $mappingData) {
            $mapping = ContractTemplatePlaceholderMapping::find($mappingData['id']);
            if ($mapping && $mapping->template_id === $template->id) {
     ...
     */
    public function applyPreset(Request $request, ContractTemplate $template, ContractTemplatePlaceholderMapping $mapping)
    {
        $this->authorize('update', $template);
     ...
     */
    public function resync(ContractTemplate $template)
    {
        $this->authorize('update', $template);
