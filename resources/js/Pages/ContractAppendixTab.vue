  ...
  <div class="card">
    ...
    <DataTable ref="dt" :value="rows" v-model:selection="selected" dataKey="id" :paginator="true" :rows="10"
      ...
      <Column field="status_label" header="Trạng thái" headerStyle="min-width:10rem;">
        <template #body="sp"><Tag :value="sp.data.status_label" :severity="statusSeverity(sp.data.status)" /></template>
      </Column>
  ...
  <Dialog v-model:visible="dialog" :style="{ width: '800px' }" header="Phụ lục hợp đồng" :modal="true">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <InputText v-model.trim="form.appendix_no" class="w-full" :invalid="submitted && !form.appendix_no || hasError('appendix_no')" />
      ...
      <div>
        ...
        <Select v-model="form.appendix_type" :options="typeOptions" optionLabel="label" optionValue="value" showClear fluid :invalid="submitted && !form.appendix_type || hasError('appendix_type')" />
      ...
      <div>
        ...
        <DatePicker v-model="form.effective_date" dateFormat="yy-mm-dd" showIcon fluid :invalid="submitted && !form.effective_date || hasError('effective_date')" />
      ...
      <div>
        ...
        <InputText type="number" v-model.number="form.insurance_salary" class="w-full" :invalid="hasError('insurance_salary')" />
      ...
      <div>
        ...
        <InputText type="number" v-model.number="form.position_allowance" class="w-full" :invalid="hasError('position_allowance')" />
...
const dt = ref(); const selected = ref([]); const dialog = ref(false); const saving = ref(false); const submitted = ref(false)
const current = ref(null); const deleting = ref(false); const deleteDialog = ref(false); const deleteManyDialog = ref(false)
const filters = ref({ global: { value: null, matchMode: 'contains' } })
const form = ref({
  id:null, appendix_no:'', appendix_type:null, effective_date:null, end_date:null,
  base_salary:null, insurance_salary:null, position_allowance:null, other_allowances:[], working_time:'', work_location:'',
  summary:'', note:''
})
const typeOptions = [
...
function openNew(){ reset(); submitted.value=false; dialog.value=true }
function edit(row){
  form.value = {
    ...row,
...
function reset(){ form.value={ id:null, appendix_no:'', appendix_type:null, effective_date:null, end_date:null, base_salary:null, insurance_salary:null, position_allowance:null, other_allowances:[], working_time:'', work_location:'', summary:'', note:'' } }
function closeDialog(){ dialog.value=false; submitted.value=false }
